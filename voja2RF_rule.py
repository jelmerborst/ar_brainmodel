...
class Voja2RF(nengo.learning_rules.LearningRuleType):
    """Vector Oja 2 learning rule version 2 - RF, only reacts on receptive fields
    ...
    def __init__(self, post_tau=0.005, learning_rate=1e-2, bias=.5):
        ...
        self.bias = bias
        super(Voja2RF, self).__init__(learning_rate, size_in=1)
...
class SimVoja2RF(Operator):
    ...
    def __init__(self, pre_decoded, post_filtered, scaled_encoders, delta,
        ...
        super(SimVoja2RF, self).__init__(tag=tag)
        self.scale = scale
...
@Builder.register(Voja2RF)
def build_voja2RF(model, voja2RF, rule):
    ...
    post = conn.post_obj
    if voja2RF.post_tau is not None:
        post_filtered = model.build(
            Lowpass(voja2RF.post_tau), model.sig[post]['out'])
    else:
    ...
    learning = Signal(np.zeros(rule.size_in), name="Voja2RF:learning")
...
#OCL - not finished
def plan_voja2RF(queue, pre, post, enc, new_enc, learn, scale, alpha, threshold, tag=None):
    ...
    )
    {
        ...
        if (i < shape0) {
            ...
            if (post > 0) {
            	...
            	new_enc[i*new_enc_stride0s[k] + j] = enc;
        	}
            //new_enc[i*new_enc_stride0s[k] + j] = alpha * learn * -post * (scale * pre - enc) + enc;
            //new_enc[i*new_enc_stride0s[k] + j] = alpha * learn * (post - sqrt(post*threshold)) * (scale * pre - enc) + enc;
            //new_enc[i*new_enc_stride0s[k] + j] = enc;
                    }
    }
    """
#sort of for loop over k, i, j #threshold, + enc
    ...
    _fn = cl.Program(queue.context, text).build().voja2RF
    _fn.set_args(*[arr.data for arr in full_args])

    lsize = None
    gsize = (new_enc.sizes.max(), N)
    plan = Plan(queue, _fn, gsize, lsize=lsize, name="cl_voja2RF", tag=tag)
...
def plan_voja2RF_prep_norm(queue, new_enc, mag, tag=None):
    ...
    text = """
    __kernel void voja2RF_prep_norm(
        __global const int *shape0s,
        __global const int *shape1s,
        __global const int *new_enc_stride0s,
        __global const int *new_enc_starts,
    ...
    _fn = cl.Program(queue.context, text).build().voja2RF_prep_norm
    _fn.set_args(*[arr.data for arr in full_args])

    lsize = None
    gsize = (mag.shape0s.max(), N)
    plan = Plan(queue, _fn, gsize, lsize=lsize, name="cl_voja2RF_prep_norm", tag=tag)
    plan.full_args = full_args     # prevent garbage-collection
...
def plan_voja2RF_apply_norm(queue, enc, new_enc, delta, mag, scale, tag=None):
    ...
    text = """
    __kernel void voja2RF_apply_norm(
        __global const int *shape0s,
        __global const int *shape1s,
        __global const int *enc_stride0s,
        __global const int *enc_starts,
    ...
    )
    {
        ...
        if (i < shape0) {
            //printf(" mag %f", mag);
            delta[i*delta_stride0s[k] + j] = scale * new_enc / mag - enc;
    ...
    _fn = cl.Program(queue.context, text).build().voja2RF_apply_norm
    _fn.set_args(*[arr.data for arr in full_args])

    lsize = None
    gsize = (delta.sizes.max(), N)
    plan = Plan(queue, _fn, gsize, lsize=lsize, name="cl_voja2RF_apply_norm", tag=tag)
    plan.full_args = full_args     # prevent garbage-collection
...
# 
#             return [
#                 plan_voja2RF(self.queue, pre, post, encoders, new_encoders, learning_signal, scale, alpha, threshold),
#                 plan_voja2RF_prep_norm(self.queue, new_encoders, mag),
#                 plan_voja2RF_apply_norm(self.queue, encoders, new_encoders, delta, mag, scale)]

...
class Mem_Voja2_Pes_Hop_TwoLayers(nengo.Network):
    def __init__(self, n_neurons=1000, n_neurons_out=1000, dimensions=None,vocab=None,
        ...
        if load_from is not None:
            ...
            new_n_neurons_out = int(data['n_neurons_out'])
            new_output_radius = data['output_radius']
            new_fwd_multi = data['fwd_multi']
            #new_fwd_matrices= data['fwd_matrices']

            
            #if pes learning not None, initialize at setting, otherwise stick with others
            if pes_rate is not None and pes_rate > 0:

                new_output_radius = output_radius
                new_fwd_multi = fwd_multi
                #new_fwd_matrices = None
                new_n_neurons_out = n_neurons_out
                if dec_ini is None:
                    decoders = data['dec']
            ...
            output_radius = new_output_radius
            fwd_multi = new_fwd_multi
            #fwd_matrices = new_fwd_matrices
            n_neurons_out = new_n_neurons_out
        ...
        with self:
            ...
            #build output layer 
            if fb > 0:
                self.output_layer = spa.State(dimensions,vocab=vocab,neurons_per_dimension=n_neurons_out, label='retrieval_out',seed=seed,feedback=fb,feedback_synapse=fb_syn)
            else:
            ...
            if pes_rate is not None and pes_rate > 0:
                ...
                nengo.Connection(self.output_layer.output, self.learn_control[:-1],
                                 synapse=None)
            ...
            if bcm_rate is not None: #recur connection
                ...
                if bcm_rate > 0:
                    ...
                    #self.conn_hop.learning_rule_type = nengo.BCM(learning_rate=bcm_rate, theta_tau=bcm_theta)
                

 
    def create_weight_probes(self,total_dur=1):
        print('total_Dur')
        print(total_dur)
        with self:
            self.probe_encoder = nengo.Probe(self.mem, 'scaled_encoders',
                                             sample_every=total_dur)
    ...
    def save(self, filename, sim):
        ...
        #print(sim.data[self.mem.output_layer])

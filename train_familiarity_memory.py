...
def load_stims(subj=0,short=True,seed=None):
    ...
    stims = np.genfromtxt(stim_path + '/stims/S' + str(subj) + 'Other.txt', skip_header=True,
                          dtype=[('probe', np.str_, 8), ('fan', 'int'), ('wordlen', np.str_, 8),
                                 ('item1', np.str_, 8), ('item2', np.str_, 8)], usecols=[1,2,3,4,5])
    stimsNFshort = np.genfromtxt(stim_path + '/stims/S' + str(subj) + 'NFShort.txt', skip_header=True,
                                 dtype=[('probe', np.str_, 8), ('fan', 'int'), ('wordlen', np.str_, 8),
                                        ('item1', np.str_, 8), ('item2', np.str_, 8)], usecols=[1,2,3,4,5])
    stimsNFlong = np.genfromtxt(stim_path + '/stims/S' + str(subj) + 'NFLong.txt', skip_header=True,
                                dtype=[('probe', np.str_, 8), ('fan', 'int'), ('wordlen', np.str_, 8),
                                       ('item1', np.str_, 8), ('item2', np.str_, 8)], usecols=[1,2,3,4,5])
...
def load_images():
    ...
    r = png.Reader(stim_path + '/images/FIXATION.png')
    r = r.asDirect()
    image_2d = np.vstack(map(np.uint8, r[2]))
    image_2d = image_2d / 255
...
#create learning model FAMILIARITY
def create_learning_model_fam(stims, reps, duration, seed=None, n_neurons=200):
    ...
    #print trial_info
    print('---- INITIALIZING LEARNING MODEL FAMILIARITY ----')
    global model 
    ...
    with model:
        ...
        def stim_func(t): #present stimulus
            if t < duration * len(stims):
                ...
                return vocab_concepts.parse('0').v
        ...
        model.mem = spa_mem_voja2_pes_hop_twolayers.SPA_Mem_Voja2_Pes_Hop_TwoLayers(
                                                ...
                                                voja2_rate=None, # NO VOJA
                                                pes_rate=0.03, #.03  
                                                output_radius=10,
                                                fwd_multi=1,
                                                fwd_dens=.05,
                                                bcm_rate=None, # NO BCM 
                                                dec_ini=0,
                                                load_from = None,
                                                seed=seed,
                                                ) 
        model.mem.mem.create_weight_probes(duration * len(stims)) #save weights
        nengo.Connection(model.stim, model.mem.input, synapse=None)
        nengo.Connection(model.correct, model.mem.correct, synapse=None)
        ...
        if nengo_gui_on:
            ...
            model.stim_state = spa.State(D,vocab= testing_vocab)
...
#check if familairity mem file exists, if so load, otherwise train mem
def train_familiarity_memory(seed=None,gui=False,ignore_mem=False,short=True):
    ...
    if short:
        fname = cur_path + '/mem_cache/familiarity_mem_twolayers_subj' + str(subj_gl) + '_' + word_length + '_' + str(seed)
    else:
        fname = cur_path + '/mem_cache/familiarity_mem_twolayers_subj' + str(subj_gl) + '_full_' + str(seed)
    
    
    if os.path.isfile(fname + '.npz') and not ignore_mem and seed != None:
        print('---- FAMILIARITY MEMORY ALREADY EXISTS, LOADING ----')
    else: #if no seed or no cache, train associative memory. 
        print('---- LEARNING FAMILIARITY MEMORY ----')
        global model
        ...
        to_be_learned_words = to_be_learned_words + tmp
        ...
        reps = 1
        dur = 1 #each presentation
        
        create_learning_model_fam(to_be_learned_words,reps,dur,seed=seed,n_neurons=30000) #stims/reps/duration
...
def create_model(seed=None,short=True):
    ...
    train_familiarity_memory(seed=seed,gui=False,ignore_mem=False,short=short)
    ...
    with model:
        ...
        model.vis_pair = spa.State(D, vocab=vocab_all_words, feedback=0, feedback_synapse=.05) 
        ...
        nengo.Connection(model.vis_pair.output,model.familiarity.input, transform=1, synapse=.05)
                                   
        model.input = spa.Input(vis_pair=vispair_func_fam)
      ...
      #probe
        ...
        nengo.Connection(model.fam_output_ens, model.familiarity_status.input, 
            function=dec_func,transform = .8)


        #vector length in output layer
        ...
        model.familiarity.mem.output_layer.state_ensembles.add_output('sumsq', sumsq)

        model.norm2 = nengo.Ensemble(n_neurons=500, dimensions=1)
        nengo.Connection(model.familiarity.mem.output_layer.state_ensembles.sumsq, model.norm2,
                        transform=np.ones( (1,model.familiarity.mem.output_layer.state_ensembles.sumsq.size_out)),synapse=None)
        ...
        if nengo_gui_on:
            ...
            for ens in model.mem_state.all_ensembles:
                ens.neuron_type = nengo.Direct()
            nengo.Connection(model.familiarity.output, model.mem_state.input,synapse=None)
        ...
        nengo.Connection(model.familiarity.mem.mem.neurons, model.act_node_ret, transform=np.ones((1, model.familiarity.mem.mem.n_neurons)),synapse=None)
        
        model.pr_familiarity = nengo.Probe(model.act_node_ret,synapse=.01) 
        ...
        model.pr_familiarity_out = nengo.Probe(model.act_node_ret_out,synapse=.01) 
        
        #retrieval decoding/confidence
        model.pr_familiarity_decoding = nengo.Probe(model.familiarity.mem.output_layer.output,synapse=.01) 
...
def vispair_func_fam(t):
    if t > .2 and t < .8:
        return '.8*(%s)+.8*(%s)' % (cur_item1, cur_item2)
    else:
        return '0'
...
def do_trial_ret(trial_info, hand):
    ...
    #judge retrieval
    ret_data = sim.data[model.pr_familiarity_decoding][400:600] #interested in 400-600 ms
    
    ind_vector = spa.similarity(ret_data, vocab_all_words)
    ...
    compare_res = sim.data[model.pr_fam_output_ens]
    ...
    results.append((subj_gl, trial_nr) + trial_info + (hand, -200, 0, vocab_all_words.keys[res], conf, compare_res, 0, vec_len))

 
    #store familiarity
    fam = sim.data[model.pr_familiarity].sum(1)
    familiarity_probe.append([subj_gl,trial_nr] + fam.tolist())
    
    #store familiarity_out
    fam_out = sim.data[model.pr_familiarity_out].sum(1)
    familiarity_out_probe.append([subj_gl,trial_nr] + fam_out.tolist())
...
def do_retrieval_test(subj=1,short=True,seed=None):
    ...
    save_probe(familiarity_probe, 'retrieval_output_familiarity_model_subj' + str(subj) + '_block' + str(bl+1))
    save_probe(familiarity_out_probe, 'retrieval_output_familiarity_out_model_subj' + str(subj) + '_block' + str(bl+1))
...
if not nengo_gui_on:
    ...
    initialize_model(subj=seed,seed=seed,short=False)
    print(vocab_concepts['METAL'].v)  
    ...
    create_model(seed=seed,short=False)

...
class Thalamus(Module):
    ...
    feedback_gates_dict : dict, optional (Default: None, no feedback on any gates)
    ...
    def __init__(self, bg, neurons_action=50, threshold_action=0.2,
                 ...
                 synapse_channel_dict=None,
                 feedback_gates_dict=None,
        ...
        self.feedback_gates_dict = feedback_gates_dict
    ...
    def get_gate(self, index, target_name):
        ...
        if index not in self.gates:
            with target_module:
                ...
                ### JPB - add feedback to gate if specified
       
                #if dict, check if specified connection exists
                if type(self.feedback_gates_dict) is dict:
                
                    #check if target_name is in dict:
                    if target_name in self.feedback_gates_dict:
                        nengo.Connection(gate, gate, transform= self.feedback_gates_dict[target_name],synapse=.04)
